...
begin
  ...
  begin
    ...
    begin
      ...
      Snippable := True;
    if (s[i] = ']') and (sb > 0) then
      sb := sb - 1;
    if (s[i] = ')') and (rb > 0) then
      rb := rb - 1;
  end;
  Replace(t, '{', '(');
  Replace(t, '}', ')');
...
begin
  ...
  begin
    ...
    for i := 0 to 3 do
      Card(i).Visible := (WOFGroup_i = 1) and TestStarted and not OptionsOnly;
    NextButton.Visible := QuestionAnswered and not TestFinished and not OptionsOnly;
    if WOFGroup_i = 0 then
      NextButton.Caption := Translate('NOE')
    else
      NextButton.Caption := Translate('NOC');

    if AssignmentExists then
      OptionsButton.Caption := Translate('SASM')
    else
    begin
      if ShowOptions then
        OptionsButton.Caption := Translate('HIDE')
      else
    ...
    begin
      ...
      begin
        repeat
          ...
          flag := True;
          for k := 0 to 3 do
            if s = Card(k).Caption then
              flag := False;
          if s = FirstWord(Answer) then
            flag := False;
        until flag;
        Card(i).Caption := s;
      end;
...
begin
  ...
  with FormQA do
  begin
    if Random < (Scrollbar1_i / 100) then
      Qlan := 1
    else
      Qlan := 2;
    if QOGroup_i = 0 then
      Qword := InUseToEntry(1)
    else
      repeat
    ...
    begin              // the vocab editor, and CurEntry is what they clicked on.
      if not IsLeft(VcbEntry[1, CurEntry], '¶') then
        Qword := CurEntry; //Check it's actual vocab and not text.
      CurEntry := 0;
    end;
    LastQuestion := Qword;
    nm := Qword;
    ...
    if IsLeft(VcbEntry[Qlan, Qword], '! ') or
      (IsLeft(VcbEntry[Qlan, Qword], '* ') and (WOFGroup_i = 0)) then
      Qlan := 3 - Qlan;
    ...
    if (WOFGroup_i = 0) and not IsLeft(VcbEntry[Qlan, Qword], '* ') then
      WrittenAnswer
    else
      ShowCards;
    HText.Caption := '';
...
begin
  ...
  for i := 0 to 3 do
    QOGroup.Items[i] := TranslateList('O1', i);
  for i := 0 to 1 do
    WOFGroup.Items[i] := TranslateList('WFW', i);
  for i := 0 to 1 do
    WOSL1Group.Items[i] := TranslateList('WSW1', i);
  for i := 0 to 1 do
    WOSL2Group.Items[i] := TranslateList('WSW2', i);
...
begin
  if AssignmentInProgress and not AssignmentHasResits and not TestFinished and
    ...
    [1, Translate('QUITA'), 2, Translate('CONTA')], '') = 2) then
    CanClose := False;
end;
...
begin
  ...
  if QAAwake and not OptionsOnly then
    RestartButtonClick(Sender);
end;
