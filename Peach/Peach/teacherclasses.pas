...
var TeachersClasses, TeachersStudents, ClassAssignments: TStrings;
...
if TeachersClasses.Count = 0 then FormStudents.LabelIntro.Caption := Translate('SIntro1a')
else FormStudents.LabelIntro.Caption := Translate('SIntro1b');
end;
...
if ApplicationGroup.ItemIndex = - 1 then ApplicationGroup.Enabled := false
   ...
   begin
   UpdateAdmissionsPolicy(Password, Username, GetItem(TeachersClasses.Strings[ClassBox.ItemIndex], ' : ', 1), policy);
   TeachersClasses.Strings[ClassBox.ItemIndex] := Copy(TeachersClasses.Strings[ClassBox.ItemIndex], 1, Length(TeachersClasses.Strings[ClassBox.ItemIndex]) - 1) + policy;
   end;
end;
...
if ClassBox.ItemIndex <> - 1 then
   begin
   if DeleteClass(Password, Username, GetItem(TeachersClasses.Strings[ClassBox.ItemIndex], ' : ', 1)) then
      begin
...
if not DeleteAssignment(Password, Username, 
      GetItem(ClassAssignments.Strings[ix], ' : ', 1))
then Exit;
...
if ClassBox.ItemIndex = - 1 then
    ...
    for i := 0 to TeachersStudents.Count - 1 do
        StudentBox.Items.Add(GetItem(TeachersStudents[i], ' : ', 1) + ' (' + GetItem(TeachersStudents[i], ' : ', 2) + ')');
    ...
    for i := 0 to ClassAssignments.Count - 1 do
        ...
        AssignmentBox.Items.Add(DateStr + ', ' + TimeStr + ' : ' + GetItem(ClassAssignments[i], ' : ', 3));
        end;
