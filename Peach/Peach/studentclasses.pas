...
begin
  cls.Free;
  while FormSClasses.ClassGrid.RowCount > 1 do
    FormSClasses.ClassGrid.DeleteRow(1);
  ...
  if cls.Count = 0 then
    FormSClasses.LabelIntro1.Caption := Translate('CIntro1a')
  else
...
begin
  ...
  if policy = 'C' then
    ShowMessage(Translate('ApC'));
  if policy = 'A' then
    SendNotification(Password, Username, EditTN.Text, '', EditCN.Text, 'r');
  if policy = 'O' then
  begin
