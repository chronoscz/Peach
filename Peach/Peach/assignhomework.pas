...
dt := EncodeDateTime(YearOf(Date), MonthOf(Date), DayOf(Date), TimeBox.ItemIndex, 0, 0, 0);
...
if CheckBox1.Checked then WriteLn(f, 'Y') else WriteLn(f, 'N');
if IsDecimal(EditLimit.Text) then WriteLn(f, EditLimit.Text) else WriteLn(f, '-1');
PutRsc(f);
WriteLn(f, '◆');
DoSave(f);
CloseFile(f);
if UploadAssignment(Password, Username, Split(TeachersClasses.Strings[ClassBox.ItemIndex], ' : ', 1), 
   FormatDateTime('yyyymmddhhmmss', dt), CheckBox1.Checked, HomeDirectory + '\Resources\File transfer\Assignment.asm', 
   EditAssignmentName.Text)
             then ShowMessage(Translate('TWHBA'));
end;
...
if CalendarDialog1.Execute then
     ...
     EditDate.Caption := FormatDateTime(Translate('Date format 1'), CalendarDialog1.Date );
     TimeBox.SetFocus;
     end;
end;
