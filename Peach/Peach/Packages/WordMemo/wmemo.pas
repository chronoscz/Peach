...
uses
  Classes, SysUtils, RichMemo, RegExpr, Graphics, Dialogs, Vocab, Windows,
  LazUTF8, LazUTF16, Keyboard, RMInlinePicture, FGL, Internet, RichMemoUtils,
  Types, CommonStringFunctions, AppLanguage;
...
const
  clPara = $00000000;    // Regular paragraph as in vocab section: can start indent.
  ...
  clPlain = $00FE0001;
  // Paragraph text in the settings: cannot start an indent.
...
begin
  ...
  InDelInline(NewPic.ip, Pos, 0, Types.Size(Round(NewPic.Pic.Width * 72 / 96),
    Round(NewPic.Pic.Height * 72 / 96)));
  Pix.Add(NewPic);
end;
...
procedure TWMemo.UpdateFont(F, TF: string; BS: integer);
var
  i: integer;
begin
...
begin
  for i := 0 to CaretPos.y do
    if GetText(XYtoX(0, CaretPos.y - i) - 1, 1) = #13 then
      break;
...
begin
  ...
  begin
    ...
    begin
      ...
      if TF.Color = clPara then
        MarkUp(XYtoX(0, y), Utf16Length(Lines[y]) + 1, psVocab);
      if TF.Color = clKBody then
        MarkUp(XYtoX(0, y), Utf16Length(Lines[y]) + 1, psKeys);
      if TF.Color = clQBody then
        MarkUp(XYtoX(0, y), Utf16Length(Lines[y]) + 1, psQForms);
      TabFlag := True;
      Key := 13;
      // OK I will freely admit that as a way of managing flow of control this may be a bit sus but
    end              // at this point it's like Caligula worrying about using the wrong fork. All the keyboard handling
    else
      Key := 0;
    // is a Lovecraftian horror anyway, let's just get it stable and never speak of it again.
  end;
  ...
  begin
    ...
    begin
      ...
      if ((TF2.Color = clVocab) or (TF2.Color = clQForms) or (TF2.Color = clKeys)) and
        (Pos(' — ', Lines[y - 1]) = 0) then
    ...
    if TF.Size > BaseFontSize then
      MarkUp(SelStart, Utf16Length(Lines[y]) - (SelStart - XYtoX(0, y)) + 1, psPara);
  end;

  if (Key = 222) and not ((TF.Color = clVocab) or (TF.Color = clKeys)) then
    // We do smart quotes.
  begin

    if SelStart > 3 then
      Pre := GetText(SelStart - 2, 1)
    else
      Pre := #13;
  ...
  if (Key = 56) and (Shift = [ssShift]) then
    // If we're at the start of a paragraph we do a bullet point for a *.
  begin
    if SelStart > 3 then
      Pre := GetText(SelStart - 2, 1)
    else
  ...
  if (TP.FirstLine = 40) and not (((Key = 0) or (Key = 8)) and not TabFlag) and
    not ((Key = Ord('Z')) and (Shift = [ssCtrl])) then
  begin
    ps := PStart;
  ...
  if (Key = 13) and (TF.Color = clPara) and IsLeft(Lines[y - 1], '•') and
    (Lines[y] = '') then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      CaretPos := TPoint.Create(0, y);
      if TF.Color = clVocab then
        MarkUp(SelStart, Utf16Length(Lines[y]) + 1, psPara);
      if TF.Color = clKeys then
        MarkUp(SelStart, Utf16Length(Lines[y]) + 1, psKBody);
      if TF.Color = clQForms then
        MarkUp(SelStart, Utf16Length(Lines[y]) + 1, psQBody);
      Key := 0;
    end;
  end;
  ...
  if (Key = 46) and ((SelStart = Utf16Length(Text)) or
    (GetUText(SelStart, 1)[1] = #13)) then
    Key := 0;
  // Do better later.
  ...
  if ((BK.Color = clFixed) or (BK.Color = clPrompt) or (BK.Color = clBool)) and
    (Key = 8) then
    Key := 0;
  ...
  begin
    ...
    begin
      if Key = 37 then
        SelStart := ls + EmdashPosition - 1;    // Left arrow.
      ...
      if not ((Key = 38) or (Key = 40)) and (cp < EmdashPosition + 2) then
        Key := 0;  // The exceptions being up and down.
    end;
    ...
    if (EmdashPosition <> 0) and (((Key = 9) and (cp < EmdashPosition)) or
      ((Key = 39) and (cp = EmdashPosition - 1))) then
    begin
      Key := 0;
      SelStart := ls + EmdashPosition + 2;
    end;

    if (EmdashPosition <> 0) and (Key = 46) and (cp = EmdashPosition - 1) then
      Key := 0;       // Delete

  end;

  if ((TF.Color = clFixed) or (TF.Color = clPrompt) or (TF.Color = clBool) or
    (TF.Color = clTitle)) and not (Key in [37 .. 40]) then
  begin
    Key := 0;
    if TF.Color = clTitle then
      Alert('CANTEDIT');
  end;
  ...
  begin
    if Key = 13 then
      Key := 0;
    if (Key = 46) and (Utf16Pos(#13, Text, SelStart) - SelStart = 1) then
      Key := 0;
  end;
...
begin
  ...
  GetTextAttributes(SelStart, TF);
  TF.Name := Font.Name;
  // Not sure why this is necessary. Possibly isn't any more, check.
  y := CaretPos.y;

  if IsLeft(Lines[y], '■') then
    InDelText('□' + Copy(Lines[y], 4, Length(Lines[y]) - 3), XYtoX(0, y),
      Utf16Length(Lines[y]))
  else
  if IsLeft(Lines[y], '□') then
    InDelText('■' + Copy(Lines[y], 4, Length(Lines[y]) - 3), XYtoX(0, y),
  ...
  begin
    ...
    begin
      InDelText('○' + Copy(Lines[y + i], 4, Length(Lines[y + i]) - 3),
        XYtoX(0, y + 1), Utf16Length(Lines[y + 1]));
  ...
  if IsLeft(Lines[y], '■') or IsLeft(Lines[y], '□') or IsLeft(Lines[y], '●') or
    IsLeft(Lines[y], '○') then
  begin
