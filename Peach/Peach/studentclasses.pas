...
for i := 0 to cls.Count - 1 do
    FormSClasses.ClassGrid.InsertRowWithValues(i + 1, [GetItem(cls.Strings[i], ' : ', 3), GetItem(cls.Strings[i], ' : ', 2)]);
if cls.Count = 0 then FormSClasses.LabelIntro1.Caption := Translate('CIntro1a') else FormSClasses.LabelIntro1.Caption := Translate('CIntro1b');
end;

procedure TFormSClasses.FormActivate(Sender: TObject);
begin
DoGetClasses;
LabelIntro2.Caption := Translate('CIntro2'); LabelIntro3.Caption := Translate('CIntro3');
...
if policy = 'A' then SendNotification(Password, Username, EditTN.Text, '', EditCN.Text, 'r');
if policy = 'O' then
   begin
...
begin
  DeleteStudentFromClass(Password, Username, GetItem(cls.Strings[ClassGrid.Row - 1], ' : ', 1));
  SendNotification(Password, Username, GetItem(cls.Strings[ClassGrid.Row - 1], ' : ', 4), GetItem(cls.Strings[ClassGrid.Row - 1], ' : ', 1), GetItem(cls.Strings[ClassGrid.Row - 1], ' : ', 2), 'x');
  DoGetClasses;
end;
