...
TestFinished: boolean; // All the questions have been answered. In this state teststarted will still be true.
...
QuestionAnswered: boolean; // The user has answered, we're waiting for them to request the next question.
...
begin
if Length(Split(s, '; ', 1)) < Length(Split(s, ', ', 1)) then FirstWord := Split(s, '; ', 1) else FirstWord := Split(s, ', ', 1);
FirstWord := Select(FirstWord); // We choose a random /-separated variant, if there is one.
end;
...
function DirectivesToQuestionFormat(j: integer; TopicDirective, EntryDirective: string): string;
var k: integer;
begin
DirectivesToQuestionFormat := Split(GetValue('DF'), ' : ', j);                          // Set to default for this .vcb file
for k := TopicStart(q_formats) to TopicEnd[q_formats] do
    if (EntryDirective = VcbEntry[1, k])
       or (MergeDirectives(TopicDirective, EntryDirective) = VcbEntry[1, k])
       or (TopicTypeWithWildcard(TopicDirective) = VcbEntry[1, k])
    then DirectivesToQuestionFormat := Split(VcbEntry[2, k], ' : ', j);
end;
...
t := ''; sb := 0; rb := 0; start := true; Snippable := true;
for i := 1 to Length(s) do
    begin
    if  s[i] = '[' then sb := sb + 1; if  s[i] = '(' then rb := rb + 1;
    ...
    if  (s[i] = ']') and (sb > 0) then sb := sb - 1; if  (s[i] = ')') and (rb > 0) then rb := rb - 1;
    end;
Replace(t, '{', '(');
Replace(t, '}', ')');
Replace(t, ' ,', ','); Replace(t, ' ;', ';'); Replace(t, ' /', '/'); Replace(t, ' |', '|');
...
begin
// strips out brackets, replaces them with a suitable amount of whitespace
t := ''; sb := 0; rb := 0; cb := 0; start := true; Snippable := true;
for i := 1 to Length(s) do
    begin
    if  s[i] = '[' then sb := sb + 1; if  s[i] = '(' then rb := rb + 1; if  s[i] = '{' then cb := cb + 1;
    ...
    if  (s[i] = ']') and (sb > 0) then sb := sb - 1; if  (s[i] = ')') and (rb > 0) then rb := rb - 1; if  (s[i] = '}') and (cb > 0) then cb := cb - 1;
    end;
Replace(t, ' ,', ','); Replace(t, ' ;', ';');
StripBrackets := t;
end;
...
begin
c := TopicEnd[seLastSetting];
while i > 0 do begin c := c + 1; if InUse[c] then i := i - 1 end;
InUseToEntry := c;
end;
...
with FormQA do
  ...
  if AssignmentInProgress then LabelC.Caption := Translate('COMT') else LabelC.Caption := Translate('COMP');
  LabelC.Visible := TestFinished and not OptionsOnly;
  QText.Visible := TestStarted and not OptionsOnly and (((Qlan = 1) and (WOSL1_i = 0)) or ((Qlan = 2) and (WOSL2_i = 0)))  ;
  SpeechButton.Visible := (TestStarted and (((Qlan = 1) and (WOSL1_i = 1)) or ((Qlan = 2) and (WOSL2_i = 1))))  and not OptionsOnly;
  HText.Visible := QuestionAnswered  and not OptionsOnly;
  AEdit.Visible := TestStarted and (WOFGroup_i = 0)  and not OptionsOnly;
  for i := 0 to 3 do Card(i).Visible := (WOFGroup_i = 1) and TestStarted  and not OptionsOnly;
  NextButton.Visible := QuestionAnswered and not TestFinished  and not OptionsOnly;
  if WOFGroup_i = 0 then NextButton.Caption := Translate('NOE') else NextButton.Caption := Translate('NOC');

  if AssignmentExists then OptionsButton.Caption := Translate('SASM')
  else
    begin
    if ShowOptions then OptionsButton.Caption := Translate('HIDE')
    else OptionsButton.Caption := Translate('SHOW')
    end;
  ...
  else LabelScore.Caption := Translate('SCORE') + 
                      ' ' + 
                      IntToStr(Qright) + 
                      '/' + 
                      IntToStr(Qaim) + 
...
if (PythonName <> '')
   and not FileExists(HomeDirectory + '\Grammar plugins\' + PythonName + '.grm') then
     begin
...
if PythonName <> '' then
  ...
  W[1] := VcbEntry[1, nm];
  W[2] := VcbEntry[2, nm];
  W[1] := TrimHint(W[1]); W[2] := TrimHint(W[2]);
  QAE := '';
  QAEnumber := - 1;
  end;
...
with FormQA do
  begin
  AEdit.Text := '';
  if RightToLeft(3 - Qlan) then AEdit.BidiMode := bdRightToLeft else AEdit.BidiMode := bdLeftToRight;
  if (Qlan = 1) then Aedit.Mode := ED_AL2 else Aedit.Mode := ED_AL1;
  AEdit.Visible := True;
  ActiveControl := AEdit;
  end;
end;
...
with FormQA do
begin
for j := 0 to 3 do begin Card(j).Color := ClWhite; Card(j).Font.Color := ClBlack end;
RightCard := Random(4);
...
for i := 0 to 3 do
    ...
    if i = RightCard then
       ...
       s := FirstWord(s);
       flag := true; for k := 0 to 3 do if s = Card(k).Caption then flag := false;
...
with FormQA do
    begin
    if Random < (Scrollbar1_i/100) then Qlan := 1 else Qlan := 2;
    if QOGroup_i = 0 then Qword := InUseToEntry(1) else
       repeat
       Qword := InUseToEntry(1 + Random(EntryNumber));
       until (EntryNumber <= 1) or (Qword <> LastQuestion);   // Just to make sure it doesn't use the same word twice.
    if CurEntry > 0 then // We have gotten here from the user clicking Test on
      ...
      if not IsLeft(VcbEntry[1, CurEntry], '¶') then Qword := CurEntry; //Check it's actual vocab and not text.
      CurEntry := 0;
      end;
    LastQuestion := Qword;
    nm := Qword;
    ...
    if IsLeft(VcbEntry[Qlan, Qword], '! ') or (IsLeft(VcbEntry[Qlan, Qword], '* ') and (WOFGroup_i = 0))
       then Qlan := 3 - Qlan;
    ...
    if (WOFGroup_i = 0) and not IsLeft(VcbEntry[Qlan, Qword], '* ') then WrittenAnswer else ShowCards;
    HText.Caption := '';
    QuestionAnswered := false; QAupdatescreen;
...
with FormQA do
  ...
  if (EntryNumber = 0) or (AssignmentExists and (Qasked = Qaim)) then TestFinished := true;
  QAupdatescreen;
  end;
...
if TestFinished and AssignmentInProgress and (Round(100*Qright/Qaim) > AssignmentScore) then
      begin
      AssignmentScore := Round(100*Qright/Qaim);
      repeat until UploadResult(Password, Username, AssignmentID, AssignmentScore);
      end;
...
var t: string;
begin
if TestFinished then begin Key := 0; Exit end;
if (Key = 13) and TestStarted and not(QuestionAnswered) then
  begin
  AEdit.SelStart := Length(AEdit.Text);
  t := Answer; Replace(t, ';', ',');
  while (t <> '') and not Match (Split(t, ', ', 1), AEdit.Text) do t := Split(t, ', ', 2);
  RightAnswer(t <> '');
  end
else
  begin
...
if (AssignmentExists and AssignmentInProgress and ((not AssignmentHasResits) or (AssignmentType = 2))) then Form1.MenuCloseClick(Sender); // This shuts down the assignment and restores the state.
TestStarted := false; TestFinished := false; AssignmentInProgress := false; AssignmentComplete := true;
if AssignmentType = 1 then DisplayVocab(DISPLAY_SELECTED); // We've had the actual vocab concealed for a half-closed test.
end;

procedure TFormQA.FormCloseQuery(Sender: TObject; var CanClose: boolean);
begin
if AssignmentInProgress and not AssignmentHasResits and not TestFinished
and (QuestionDlg(Translate('AREYOUSURE'), Translate('NORESIT'), mtCustom, [1, Translate('QUITA'), 2, Translate('CONTA')], '') = 2)
         then CanClose := false;
end;

procedure TFormQA.NextButtonClick(Sender: TObject);
begin
AskQuestion; QuestionAnswered := false
end;
...
TestStarted := true; TestFinished := false; QAUpdateScreen;
AskQuestion;
end;
...
begin
if CheckCFBT.Checked[0] then CheckCFBT_i := 1 else CheckCFBT_i := 0;
