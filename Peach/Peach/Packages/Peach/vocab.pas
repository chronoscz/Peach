...
begin
  ...
  begin
    ...
    then
      EntryToShortString := VcbEntry[1, j]
    else
  ...
  if (VcbEntry[1, j] = '') and (VcbEntry[2, j] = '') and (Qtype[j] = '') then
    EntryToShortString := '';
  if j >= TopicStart(VocabStarts) then
    EntryToShortString := StripSquareBrackets(EntryToShortString);
end;
...
function MergeDirectives(ttype, Qtype: string): string;
begin
  if (ttype = '') or (Qtype = '') then
    MergeDirectives := ttype + Qtype
  else
...
begin
  ...
  begin
    ...
    t := Split(t, '/', 2);
    Replace(u, '_', ' ');
    if (u = candidate) and (Expression = '') then
      Match := True
...
begin
  if i = 1 then
    KeyCount := TopicSize(seLang1Keys)
  else
...
begin
  if i = 1 then
    Keys := VcbEntry[k, TopicStart(seLang1Keys) + j - 1]
  else
...
begin
  HasVocab := False;
  for i := TopicStart(j) to TopicEnd[j] do
    HasVocab := HasVocab or IsVocab(i);
end;
...
begin
  ...
  for k := seDatastarts to LastTopic do
    if (TopicEnd[k - 1] < i) and (i <= TopicEnd[k]) then
      EntryToTopic := k;
end;
...
begin
  ...
  begin
    if i = 1 then
      AudioID := Split(GetValue('Aud1'), ' : ', 1);
    if i = 2 then
      AudioID := Split(GetValue('Aud2'), ' : ', 1);
  end
  else
    AudioID := '';
end;
...
begin
  ...
  begin
    if i = 1 then
      Language := GetValue('Lang1');
    if i = 2 then
      Language := GetValue('Lang2');
  end
  else
...
begin
  ...
  for i := seDatastarts to LastTopic do
    if TopicsToDisplay[i] then
      TopicDisplayStr := TopicDisplayStr + 'Y'
    else
