...
uses
  ...
  LazUTF8, LazUTF16, ClipBrd, Crt, CommonStringFunctions;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        temps := GetText(i, 1);
        if temps = CloseBracket then
          BracketCount := BracketCount + 1;
        if temps = OpenBracket then
          BracketCount := BracketCount - 1;
        GetTextAttributes(i, TF);
        if TF.Style = [fsBold] then
          break;
        Dec(i);
      end;
...
{if (Key=13) and (y>0) and (Lines[y-1]<>'') then          // enter
  ...
  begin
    ...
    begin
      ...
      if Lines[y - 1][Length(Lines[y - 1])] = ':' then
        temps := #9 + temps;
...
{if (Key=8) and (x>0) and (copy(Lines[y],1,x) = stringofchar(' ',x)) and OldBlank then           // backspace
  ...
  begin
    ...
    begin
      ...
      if (not q1) and (not q2) and not Found then
        Quoted := '';
