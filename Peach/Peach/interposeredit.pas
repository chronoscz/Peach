...
begin
  ...
  if self.Mode = ED_NORMAL then
    Exit;

  if self.Mode in [ED_AL1, ED_AL2] then
  begin
    if self.Mode = ED_AL1 then
      self.Text := AddAccent(self.Text, 1);
    if self.Mode = ED_AL2 then
      self.Text := AddAccent(self.Text, 2);
  end;
...
begin
  if (self.Mode in [ED_AL1, ED_AL2]) and (Msg.Msg = WM_PASTE) then
    Msg.Msg := WM_NULL;
  inherited;
end;
