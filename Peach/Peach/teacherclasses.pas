...
begin
  ...
  if TeachersClasses.Count = 0 then
    FormStudents.LabelIntro.Caption := Translate('SIntro1a')
  else
...
begin
  ...
  for i := 0 to 2 do
    ApplicationGroup.Items[i] := TranslateList('Ad1', i);
...
begin
  ...
  if ApplicationGroup.ItemIndex = -1 then
...
begin
  ...
  if not DeleteAssignment(Password, Username,
    GetItem(ClassAssignments.Strings[ix], ' : ', 1)) then
    Exit;
...
begin
  ...
  begin
    ...
    if policy = 'O' then
      ApplicationGroup.ItemIndex := 0;
    if policy = 'A' then
      ApplicationGroup.ItemIndex := 1;
    if policy = 'C' then
      ApplicationGroup.ItemIndex := 2;
  end;
end;
