      ...
      begin
        ...
        begin
          ...
          begin   // Not paragraph text.
            if (i >= VocabStarts) or
              (i in [q_formats, seLang1Keys, seLang2Keys]) then
            begin
              ...
              if (i >= VocabStarts) then
                WMemo.MarkUp(UtfStart, UtfPlus, psVocab);
              if (i in [seLang1Keys, seLang2Keys]) then
...
begin
  ...
  begin
    ...
    begin
      ...
      if Filename = '' then
        VHeader.Caption := ' ' + Translate('UNTF')
      else
    ...
    if not MenuMainScreen.Checked then
      VHeader.AnchorParallel(akLeft, 12, Form1);
    if not MenuMainScreen.Checked then
      VHeader.AnchorParallel(akRight, 12, Form1);
    ...
    if AssignmentComplete then
      ButtonAssignment.Caption := Translate('ButAsmR')
    else
      ButtonAssignment.Caption := Translate('ButAsm');

    WMemo.ReadOnly := (ViewMode = VIEW_MAIN);

    if TopicBox.Visible then
      TopicBox.Enabled := not AssignmentExists;
    ...
    if MenuOptions.Visible then
      MenuOptions.Enabled := not AssignmentExists;
    if MenuView.Visible then
      MenuView.Enabled := not AssignmentExists;
    if MenuUser.Visible then
      MenuUser.Enabled := not AssignmentExists;
    if MenuClassroom.Visible then
      MenuClassroom.Enabled := not AssignmentExists;
...
begin
  ...
  if MenuOpen.Visible then
    MenuOpen.Enabled := not TestStarted;
  if MenuSave.Visible then
    MenuSave.Enabled := (MenuEditGrm.Checked and GrmFileExists and
      GrmUnsavedChangesExist) or ((not MenuEditGrm.Checked) and DataExists and
      VcbFileExists and UnsavedChangesExist);
  if MenuSaveAs.Visible then
    MenuSaveAs.Enabled := (MenuEditGrm.Checked) or
      ((not MenuEditGrm.Checked) and DataExists);
  if MenuRevert.Visible then
    MenuRevert.Enabled := (MenuEditGrm.Checked and GrmFileExists and
      GrmUnsavedChangesExist) or ((not MenuEditGrm.Checked) and DataExists and
      VcbFileExists and UnsavedChangesExist and not TestStarted);
  if MenuNewFileKeepSettings.Visible then
    MenuNewFileKeepSettings.Enabled := DataExists and not TestStarted;
  if MenuNewFileNewSettings.Visible then
    MenuNewFileNewSettings.Enabled := not TestStarted;
  MenuNewGrm.Visible := MenuEditGrm.Checked;
  if MenuClose.Visible then
    MenuClose.Enabled := (MenuEditGrm.Checked) or ((not MenuEditGrm.Checked) and DataExists);
end;
...
begin
  if MenuEditSettings.Visible then
    MenuEditSettings.Enabled := DataExists;
  if MenuEditVcb.Visible then
    MenuEditVcb.Enabled := DataExists;
end;
...
begin
  ...
  if GKeyGroup.Checked[0] then
    GMemo.Mode := MM_L1;
  if GKeyGroup.Checked[1] then
    GMemo.Mode := MM_L2;
end;
...
begin
  ...
  if not IsAudio[1] then
    WOSL1_i := 0;
  if not IsAudio[2] then
    WOSL2_i := 0;
end;
...
begin
  ...
  if (Screen.Fonts.IndexOf(GetValue('POFont')) <> -1) then
    MainFont := GetValue('POFont');
  if (Screen.Fonts.IndexOf(GetValue('TIFont')) <> -1) then
    TitleFont := GetValue('TIFont');
  if IsDecimal(GetValue('POSize3')) then
    BaseFontSize := StrToInt(GetValue('POSize3'));
  if IsHex(GetValue('BCOLOR')) then
    Form1.Color := Hex2Dec(GetValue('BCOLOR'));

  if BooleanValue('01') then
    QOGroup_i := 0;
  if BooleanValue('02') then
    QOGroup_i := 1;
  if BooleanValue('03') then
    QOGroup_i := 2;
  if BooleanValue('04') then
    QOGroup_i := 3;

  if BooleanValue('WSW1') then
    WOSL1_i := 0;
  if BooleanValue('WSS1') then
    WOSL1_i := 1;

  if BooleanValue('WSW2') then
    WOSL2_i := 0;
  if BooleanValue('WSS2') then
    WOSL2_i := 1;

  if BooleanValue('WFW') then
    WOFGroup_i := 0;
  if BooleanValue('WFF') then
    WOFGroup_i := 1;

  if BooleanValue('TO3Y') then
    CheckCFBT_i := 1;
  if BooleanValue('TO3N') then
    CheckCFBT_i := 0;
  ...
  for i := 2 to LockdownCount do
    if BooleanValue('Lo' + IntToStr(i)) then
      LockStr[i] := 'Y';
...
begin
  ...
  begin
    ...
    if ViewMode = VIEW_EDIT then
      SchlurpWMemo;
    TopicName[VocabStarts] := Split(ExtractFilename(Filename), '.', 1);
    MakeTopicBox;
    if ViewMode = VIEW_SETTINGS then
      SchlurpSettings;
    SaveAll;
    VcbFileExists := True;
    UnsavedChangesExist := False;
...
begin
  ...
  with Form1 do
  begin
    if UpdateSetting = 0 then
      MenuAlwaysUpdate.Checked := True;
    if UpdateSetting = 1 then
      MenuNeverUpdate.Checked := True;
    if UpdateSetting = 2 then
      MenuAlwaysAsk.Checked := True;

    if VersionSetting = 0 then
      MenuLatestVersion.Checked := True;
    if VersionSetting = 1 then
      MenuStableVersion.Checked := True;
  end;
...
begin
  ...
  begin
    ...
    GrammarCheck := True;
  if not GrammarCheck then
    Form1.MenuEditGrm.Checked := True;
end;
...
begin
  ...
  if Memo.SelText <> '' then
    FormFind.EditFind.Text := Memo.SelText;
  FormFind.ShowModal;
...
begin
  ...
  begin
    ...
    if not ((Shift = [ssCtrl]) and ((ch = 67) or (ch = 86) or
      ...
      begin
        if (ch in [65..90]) and not (ssShift in Shift) then
          ch := ch + 32;
  ...
  if TopicCombo.ItemIndex <> -1 then
    for i := TopicStart(VocabStarts + TopicCombo.ItemIndex)
      ...
      if not IsLeft(VcbEntry[1, i], '¶') then
        WordCombo.Items.Add(EntryToString(i));
  WordCombo.SetFocus;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ix <> -1) and (DataEdit.Text <> WordCombo.Items[ix]) then
        ix := -1;
...
begin
  ...
  for j := 0 to LastTopic do
    TopicsToDisplay[j] := False;
  c := 0;
  LastTopic := BASEMENT;
  j := 0;
  TopicEnd[LastTopic] := j;
...
begin
  ...
  begin
    for i := 1 to FormAssignments.AGrid.RowCount - 1 do
      if FormAssignments.AGrid.Cells[0, i] = '1' then
        n := i - 1;
...
begin
  ...
  begin
    ...
    begin
      ...
      SetFonts;
      if MenuEditSettings.Checked or MenuEditVcb.Checked then
        DisplayVocab(DISPLAY_ALL);
      if MenuMainScreen.Checked then
        DisplayVocab(DISPLAY_SELECTED);
      MakeTopicBox;
...
begin
  ...
  begin
    ...
    for i := 0 to LockdownCount - 1 do
      LockdownGroup.Items[i] := TranslateList('Lo1', i);
    for i := 0 to LockdownCount - 1 do
      LockdownGroup.Checked[i] := (LockStr[i + 1] = 'Y');
    Button1.Caption := Translate('OK');
    Button2.Caption := Translate('CANCEL');
    ShowModal;
    if ModalResult = mrOk then
    begin
      for i := 0 to LockdownCount - 1 do
        if LockdownGroup.Checked[i] then
          LockStr[i + 1] := 'Y'
        else
          LockStr[i + 1] := 'N';
      LockdownBackup := LockStr;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (i = q_formats) and (j = TopicStart(q_formats) + 1) then
              VcbEntry[1, j] := '#[DF]';
          end;
    ...
    begin
      ...
      begin
        ...
        if AssignmentType = 1 then
          DisplayVocab(DISPLAY_NONVOCAB);  // It's a half-closed test, we're closing the book.
      end
      else
      begin
      ...
      for i := 0 to 3 do
        Card(i).Font.Name := MainFont;
      for i := 0 to 3 do
        Card(i).Height := 2 * Card(i).Canvas.TextHeight('Hey');
      for i := 0 to 3 do
        Card(i).Font.Size := Round(BaseFontSize * 1.33);
...
begin
  ...
  begin
    ...
    for i := 0 to TopicBox.Items.Count - 1 do
      TopicBox.Checked[i] := TopicsToDisplay[IndexToTopic(i)];
