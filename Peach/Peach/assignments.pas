...
procedure TFormAssignments.AGridSelectCell(Sender: TObject; ACol, 
...
for i := 0 to AssignmentList.Count - 1 do
    ...
    if IsDecimal(tempsc) then tempsc := tempsc + '%' else tempsc := '';
    AGrid.InsertRowWithValues(i + 1, 
        [
        '0', 
        GetItem(AssignmentList.Strings[i], ' : ', 2), 
        GetItem(AssignmentList.Strings[i], ' : ', 5), 
        DateStr, 
        TimeStr, 
        tempsc
        ]);
