...
VcbFileExists: boolean; // True if the data's been saved at least once and so has a filename.
...
SomethingChecked: boolean; // True if at least one topic has been checked, so the test can be started.
...
t := ''; sb := 0; start := true; Snippable := true;
...
function EntryToShortString(j: integer): string;
var temps: array[1..2] of string;
    i: integer;
begin
if j < TopicStart(VocabStarts) then
   begin
   if (TopicStart(seAboutVlist) <= j) and (j <= TopicEnd[seKeysHelp])
   or (TopicStart(seTestOverrides) <= j) and (j <= TopicEnd[seTestOverrides] - 2)
   or (TopicStart(seLockdownOverrides) <= j) and (j <= TopicEnd[seLockdownOverrides])
      then EntryToShortString := VcbEntry[1, j]
   else  EntryToShortString := VcbEntry[1, j] + ' — ' + VcbEntry[2, j];
   end
else         // This is a regular vocab item ...
    begin
    for i := 1 to 2 do
        begin
        temps[i] := Split(VcbEntry[i, j], '; ', 1);     // So we remove the hidden synonyms ...
        temps[i] := TrimHint(temps[i]); // And any hints for the question generator.
        end;
    EntryToShortString := temps[1] + ' — ' + temps[2];
    end;
if (VcbEntry[1, j] = '') and (VcbEntry[2, j] = '') and (Qtype[j] = '')
   then EntryToShortString := '';
if j >= TopicStart(VocabStarts)
   then EntryToShortString := StripSquareBrackets(EntryToShortString);
end;
...
if IsLeft(s, '[') and (Pos('] ', s) > 0) and (tn >= VocabStarts) then   // Why not calculate whether we're in vocab from i? Because we might be reading the file.
   begin
   Qtype[i] := Split(s, '] ', 1) + ']';
   s := TrimDirective(s);
...
if IsLeft(s, '¶') then VcbEntry[1, i] := s else VcbEntry[1, i] := Split(s, ' — ', 1); VcbEntry[2, i] := Split(s, ' — ', 2);
end;
...
var s, t: string;
begin
if i >= 0 then s := TopicName[i] else s := '';
...
else MergeDirectives := Split(ttype, ']', 1) + ', ' + Split(Qtype, '[', 2)
end;
...
while t <> '' do
      ...
      if (u = candidate) and (Expression = '') then Match := true
        ...
        Match := Match(Expression, Copy(candidate, Length(u) + 2, Length(candidate) - Length(u) - 1));
      end;
end;
...
i := 1; repeat
        ...
        if (Copy(s, i, Length(t)) = t) and (sb + rb + cb = 0) then
           begin
           s := Copy(s, 1, i - 1) + u + Copy(s, i + Length(t), 1 + Length(s) - i - Length(t));
...
begin
if i = 1 then KeyCount := TopicSize(seLang1Keys) else KeyCount := TopicSize(seLang2Keys);
end;

function Keys(i, j, k: integer): string;
begin
if i = 1 then Keys := VcbEntry[k, TopicStart(seLang1Keys) + j - 1] else Keys := VcbEntry[k, TopicStart(seLang2Keys) + j - 1];
end;
...
var j: integer;
begin
s := ' ' + s + ' ';        // So that the start and the end of the string look like word boundaries.
for j := 1 to KeyCount(lg) do
    ReplaceButBrackets(s, Keys(lg, j, 1), Keys(lg, j, 2)) ;
Replace(s, 'ᵃ', ''); Replace(s, 'ᵇ', ''); Replace(s, 'ᶜ', '');
s := Copy(s, 2, Length(s) - 2);
AddAccent := s;
end;

function IsVocab(i: integer): boolean; // Identifies whether an entry is a testable vocab item.
begin
...
function TopicStart(i: integer): integer;
begin TopicStart := TopicEnd[i - 1] + 1 end;

function EntryToTopic(i: integer): integer;
var k: integer;
begin
EntryToTopic := - 1; for k := seDatastarts to LastTopic do if (TopicEnd[k - 1] < i) and (i <= TopicEnd[k]) then EntryToTopic := k;
end;
...
else Language := TranslateList('Lang1', i - 1);
end;
...
t := ''; if Qtype[i] <> '' then t := Qtype[i] + ' ';
if ((i >= TopicStart(seAboutVlist)) and (i <= TopicEnd[seKeysHelp]))
   then EntryToString := t + VcbEntry[1, i]
   else EntryToString := t + VcbEntry[1, i] + ' — ' + VcbEntry[2, i];
...
with Sender as TEdit do
      ...
      if (ch in [65..90]) or (ch in [48..57]) then
           ...
           if s <> '' then
              ...
              Text := Utf8Copy(Text, 1, Utf8x) + s + Utf8Copy(Text, Utf8x + 1, Utf8Length(Text) - Utf8x);
...
for i := seDatastarts to LastTopic do
    if TopicsToDisplay[i] then TopicDisplayStr := TopicDisplayStr + 'Y'
        else TopicDisplayStr := TopicDisplayStr + 'N';
...
WriteLn(f, Qlan - 1);
if Qlan = 1 then temp := WOSL1_i else temp := WOSL1_i;
if temp = 0 then WriteLn(f, 'written') else WriteLn(f, 'spoken');
if WOFGroup_i = 0 then WriteLn(f, 'written') else WriteLn(f, 'multiple choice');
if ix = - 1 then        // In this case, we are using the IDE and have passed a string not in the vocab.
   begin
   if IsLeft(s, '[') and (Pos('] ', s) > 0) then
        begin
        Qf := Split(s, '] ', 1) + ']';
