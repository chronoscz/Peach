...
UpdatesExist := false;
AvailableUpdates := GetListFromUrl('https://github.com/peachpit-site/peach-updates/releases/download/'+VersionName+'/updates.txt');
...
if UpdatesExist then
     ...
     if UpdateSetting = 2 then
        begin
        FormUpdate.Memo1.Text := GetStringFromUrl('https://github.com/peachpit-site/peach-updates/releases/download/'+VersionName+'/changes.txt');
        for i := 0 to 1 do
     ...
     for i := 0 to AvailableUpdates.Count - 1 do
         ...
         if Newer(NewVersion,Oldversion) then
            ...
            if UpdateFileName = 'Peach.exe' then
               ...
               UpdateIcon(hd + 'Peach\' + ExeName + '.exe', hd + 'Peach\Resources\Icon\icon.ico');
              ...
              GetFileFromUrl('https://github.com/peachpit-site/peach-updates/releases/download/'
                  + VersionName + '/' + UpdateFileName, PChar(hd+'Peach\' + UpdateFilePath));
            end;
         end;
     end;
AvailableUpdates.Free;
...
AssignFile(f, PChar(ExtractFilePath(ParamStr(0))+'\Peach\Resources\Rsc files\data1.rsc'));
Reset(f);
Readln(f); Readln(f); Readln(f); Readln(f); Readln(f, applang);
CloseFile(f);

GetLanguage(PChar(ExtractFilePath(ParamStr(0))+'Peach\Internationalization\'+applang+'.lng'));
