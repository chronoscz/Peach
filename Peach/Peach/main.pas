...
uses
  ...
  Assignments, TestResults, InterposerEdit, PyMemo, WMemo, RichMemoUtils, 
  RMInlinePicture, IconUpdater, VersionSupport, CommonStringFunctions, 
  AppLanguage, FontInfo, Licenses, PythonFunctions;
...
var LowerTopic, UpperTopic: integer;
...
GrmFileExists: boolean; // True if the data's been saved at least once and so has a filename.
...
LockStr: string; // The user's saved lockdown preferences in the form of a string of Ys and Ns.
...
const FROM_DISC = false;
      FROM_MEMO = true;

var Scheme: TStrings; // This will hold the contents of the file settings_scheme.rsc,
...
until (BodyStarts = Form1.GMemo.Lines.Count)
      or ((Form1.GMemo.Lines[BodyStarts] <> '')
      and (Form1.GMemo.Lines[BodyStarts][1] <> ' ')
      and (Form1.GMemo.Lines[BodyStarts][1] <> '#')
      and not (IsLeft('def', Form1.GMemo.Lines[BodyStarts])));
end;

function IndexToTopic(i: integer): integer;
begin
if i = - 1 then IndexToTopic := - 1 else IndexToTopic := LowerTopic + i;
end;
...
var i, j, MaxW: integer;
begin
with Form1 do begin

//We figure out how large it should be to accommodate the topic/property names
Canvas.Font := Font;
MaxW := 0; for i := seDatastarts to LastTopic do MaxW := Max(MaxW, 60 + TopicBox.Canvas.TextWidth(TrimDirective(TopicName[i])));
THeader.Width := MaxW;
...
with Form1 do
  ...
  WMemo.UpdateFont(MainFont, TitleFont, BaseFontSize);
...
with Form1 do
  ...
  for i := LowerTopic to UpperTopic do
      if (DisplayMode = DISPLAY_ALL)
         or (TopicsToDisplay[i] and
            ((DisplayMode = DISPLAY_SELECTED)
             or ((DisplayMode = DISPLAY_NONVOCAB) and not HasVocab(i)))) // For closed-book tests.
      then
           begin
           ...
           if (0 < i) and (i < VocabStarts) then WMemo.MarkUp(UtfStart, UtfPlus, psSetting);
           if i = VocabStarts then WMemo.MarkUp(UtfStart, UtfPlus, psTitle);
           if i = seSettingsHeader then WMemo.MarkUp(UtfStart, UtfPlus, psSettingsHeader);
           ...
           for j := TopicStart(i) to TopicEnd[i] do
               ...
               if IsLeft(EntryToString(j), '¶') then
                     begin
                     StrPlus := Decode(Utf8Copy(EntryToString(j), 2, Length(EntryToString(j)) - 1), '#');
                     if IsLeft(StrPlus, 'img : ') and (ViewMode = VIEW_MAIN) then
                         ...
                         if not ((i >= VocabStarts) or (i in [seAboutVlist, seKeysHelp, 
                            seLang1Keys, seLang2Keys, q_formats])) then
                  ...
                  if (i >= VocabStarts) or (i in [q_formats, seLang1Keys, seLang2Keys]) then
                     begin
                     ...
                     if i = q_formats then WMemo.MarkUp(UtfStart, UtfPlus, psQForms);
                     if (i >= VocabStarts) then WMemo.MarkUp(UtfStart, UtfPlus, psVocab);
                     if (i in [seLang1Keys, seLang2Keys]) then WMemo.MarkUp(UtfStart, UtfPlus, psKeys);
                  ...
                  if not((i >= VocabStarts) or (i in [q_formats, seLang1Keys, seLang2Keys])) then
                     begin
                     StrPlus := Decode(EntryToShortString(j), '#');
                     WMemo.Lines.Add(StrPlus);
                     UtfPlus := Utf16Length(StrPlus) + 1;
                     ...
                     if EmdashPosition = 0 then
                        ...
                        if IsLeft(StrPlus, '■') or IsLeft(StrPlus, '□')
                        or IsLeft(StrPlus, '●') or IsLeft(StrPlus, '○') then
                           WMemo.Markup(UtfStart, 2, psBool);
                        end
                     else
                        begin
                        WMemo.MarkUp(UtfStart, EmdashPosition + 1, psPrompt);
                        WMemo.MarkUp(UTFStart + EmdashPosition + 1, UtfPlus - EmdashPosition - 1, psEntry);
                        end;
                     end;
                  UtfStart := UtfStart + UtfPlus;
  ...
  if MenuEditVcb.Checked and (UpperTopic = VocabStarts) and (TopicSize(VocabStarts) = 0) then
      begin    // ... then we need somewhere for the user to start typing.
      WMemo.Lines.Add('');
      WMemo.Markup(UtfStart - 1, 1, psFixed);
      end;
  if i - LowerTopic < TopicBox.ItemIndex then ScrollTo := WMemo.Lines.Count - 1;
...
with Form1 do
  ...
  DataEdit.Visible := MenuEditGrm.Checked;   // If you put this further down it conflicts with making the topic box and word box visible fml.
  ...
  if MenuMainScreen.Checked or MenuEditVcb.Checked then
     ...
     else VHeader.Caption := ' ' + Split(ExtractFileName(Filename), '.', 1);
     end;
  ...
  TestButton.Visible := MenuMainScreen.Checked and SomethingChecked and not AssignmentExists and not TestStarted;

  ButtonPractice.Visible := MenuMainScreen.Checked and SomethingChecked and AssignmentExists and (AssignmentType <> 3) and not TestStarted;
  ButtonAssignment.Visible := MenuMainScreen.Checked and SomethingChecked and AssignmentExists and not TestStarted;

  if AssignmentComplete then ButtonAssignment.Caption := Translate('ButAsmR') else ButtonAssignment.Caption := Translate('ButAsm');

  WMemo.ReadOnly := (ViewMode = VIEW_MAIN);

  if TopicBox.Visible then TopicBox.Enabled := not AssignmentExists;

  WMemo.Visible := (MenuMainScreen.Checked or MenuEditVcb.Checked or MenuEditSettings.Checked) and DataExists;
  ...
  MenuOptions.Visible := (LockStr[lkTestOptions] = 'N')
                      or (LockStr[lkAppearanceOptions] = 'N')
                      or (LockStr[lkUpdateOptions] = 'N')
                      or (LockStr[lkLockdownOptions] = 'N');
...
MenuPublish.Visible := not ((LockStr[lkRestrictFileMenu] = 'Y') or (LockStr[lkPublish] = 'Y') or AssignmentExists);
MenuNew.Visible := not (MenuEditGrm.Checked or (LockStr[lkRestrictFileMenu] = 'Y') or AssignmentExists or TestStarted);
if MenuOpen.Visible then MenuOpen.Enabled := not TestStarted;
if MenuSave.Visible then MenuSave.Enabled := (MenuEditGrm.Checked and GrmFileExists and GrmUnsavedChangesExist)
   or ((not MenuEditGrm.Checked) and DataExists and VcbFileExists and UnsavedChangesExist);
if MenuSaveAs.Visible then MenuSaveAs.Enabled := (MenuEditGrm.Checked) or ((not MenuEditGrm.Checked) and DataExists);
if MenuRevert.Visible then MenuRevert.Enabled := (MenuEditGrm.Checked and GrmFileExists and GrmUnsavedChangesExist)
   or ((not MenuEditGrm.Checked) and DataExists and VcbFileExists and UnsavedChangesExist and not TestStarted);
if MenuNewFileKeepSettings.Visible then MenuNewFileKeepSettings.Enabled := DataExists and not TestStarted;
...
if MenuClose.Visible then MenuClose.Enabled := (MenuEditGrm.Checked) or ((not MenuEditGrm.Checked) and DataExists);
end;
...
procedure TForm1.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; x, y: Integer);
begin
  ShowMessage(HomeDirectory);
ShowMessage(Copy(HomeDirectory, Length(HomeDirectory) - 5, 6));
end;
...
SelectedEntry := 0; SelectedTopic := BASEMENT;
...
else for i := 0 to Form1.GMemo.Lines.Count - 1 do WriteLn(f, '    ' + Form1.GMemo.Lines[i]);
...
for i := VocabStarts to LastTopic do
    if (FileExists(HomeDirectory + 'Grammar plugins\' + TopicNumberToGrmFilename(i) + '.grm'))
    and (Converted.IndexOf(TopicNumberToGrmFilename(i)) = - 1) then
       begin
       ...
       if E <> '' then
          begin
          AssignFile(f, HomeDirectory + 'Resources\Py files\' + TopicNumberToGrmFilename(i) + '.py');
          Rewrite(f);
          WriteLn(f, 'import os');
          WriteLn(f, 'import io');
...
if (Screen.Fonts.IndexOf(GetValue('TIFont')) <> - 1) then TitleFont := GetValue('TIFont');
...
if IsDecimal(GetValue('TO4'))
and (StrToInt(GetValue('TO4')) in [0..100])
  then Scrollbar1_i := StrToInt(GetValue('TO4'));
...
Reset(f);
LastTopic := BASEMENT; j := 0; TopicEnd[LastTopic] := j;
while not eof(f) do
   begin
   ReadLn(f, s);
...
ConvertAllGrmToPy;
ValidateAudio;
Form1.MenuMainScreen.Checked := true; LowerTopic := VocabStarts; UpperTopic := LastTopic;
MakeTopicBox; DoOverrides; DisplayVocab(DISPLAY_SELECTED);
DataExists := true; VcbFileExists := true; UnsavedChangesExist := false; UpdateScreen;
...
with Form1 do
  ...
  MaxW := Max(Form1.Canvas.TextWidth(GetDataLabel.Caption), Form1.Canvas.TextWidth(DataLabel.Caption));
...
QOGroup_b := QOGroup_i; WOSL1_b := WOSL1_i; WOSL2_b := WOSL2_i; WOFGroup_b := WOFGroup_i; CheckCFBT_b := CheckCFBT_i; Scrollbar1_b := Scrollbar1_i;
Mainfont_b := MainFont; TitleFont_b := TitleFont; Fontsize3_b := BaseFontSize; LockdownBackup := LockStr;
...
begin
  ...
  UpdateScreen;
  SaveAs := true;
  if (Form1.MenuMainScreen.Checked and TopicsToDisplay[VocabStarts]) or Form1.MenuEditVcb.Checked then
     begin
     Form1.WMemo.Lines.BeginUpdate;
     Form1.WMemo.Lines[1] := TopicName[VocabStarts];
     Form1.WMemo.MarkUp(Form1.WMemo.XYtoX(0, 1), Utf16Length(Form1.WMemo.Lines[1]), psTitle);
...
CloseFile(f);
GrmFileExists := true; GrmUnsavedChangesExist := false; UpdateScreen;
end;
...
if Filename <> '' then if Filename[1] = ':' then Filename := HomeDirectory + 'Vocab lists\' + Copy(Filename, 2, Length(Filename) - 1);
OpenLanguage;
LastTopic := 0;
DataExists := false;
Wipe;
...
if Filename = '' then
     ...
     begin
     DataExists := true; VcbFileExists := true; OpenVcbFile;
...
if (Username <> '') and (Password <> '') and ValidateUser(Username, Password) then
  begin
  t := Get_names(Password, Username);  // Retrieves the first and last name of the user from the database.
  FirstName := Split(t, ' : ', 1);
  LastName := Split(t, ' : ', 2);
  SignedIn := true;
  end;
...
begin
if DataExists and UnsavedChangesExist then begin
case QuestionDlg(Translate('AREYOUSURE'), Translate('UNSAVED'), mtCustom, [1, Translate('SAVE'), 2, Translate('DONTSAVE'), 3, Translate('CANCEL')], '') of
    1: if Filename = '' then begin AreYouSure := SaveAs end else begin SaveAll; AreYouSure := true; UnsavedChangesExist := false; UpdateScreen;   end;
    2: begin AreYouSure := true; DataExists := false; UnsavedChangesExist := false; UpdateScreen end;
    3: AreYouSure := false;
    end;
    end
...
if (ViewMode = VIEW_GRAMMAR) and GrmUnsavedChangesExist then
   begin
   case QuestionDlg(Translate('AREYOUSURE'), Translate('UNSAVEDG'), mtCustom, [1, Translate('SAVE'), 2, Translate('DONTSAVE'), 3, Translate('CANCEL')], '') of
        1: if Filename = '' then
              GrammarCheck := SaveGrmAs
           else
              begin SaveGrm; GrammarCheck := true end;
        2: GrammarCheck := true;
        3: GrammarCheck := false;
        end;
    end
else GrammarCheck := true;
...
CloseFile(f);
GrmFileExists := true; GrmUnsavedChangesExist := false;
UpdateScreen;
...
with Form1 do
      ...
      temp := FindText(Memo.Text, FormFind.EditFind.Text, Memo.SelStart + Memo.SelLength + 1, FormFind.CheckCase.Checked) - 1;
      if temp < 0 then
         begin
         if QuestionDlg(Translate('NO FIND'), Translate('RESEARCH'), mtCustom, [1, Translate('REYES'), 2, Translate('RENO')], '') = 1 then
              begin
              temp := FindText(Memo.Text, tempfind, 1, FormFind.CheckCase.Checked) - 1;
              if temp < 0 then Alert('NO FIND') else
                  begin
...
if FormFind.ModalResult = mrOK then
     ...
     temp := FindText(Memo.Text, FormFind.EditFind.Text, 1, FormFind.CheckCase.Checked) - 1;
     if temp < 0 then Alert('NO FIND') else
        begin
        UnsavedChangesExist := true;
          ...
          temp := FindText(Memo.Text, FormFind.EditFind.Text, temp + Length(FormFind.EditReplace.Text) - Length(FormFind.EditFind.Text) + 2, FormFind.CheckCase.Checked) - 1;
          until temp < 0;
        end;
...
begin
if (not (Key in [37..40]))
    and (not MenuMainScreen.Checked)
        then{} UnsavedChangesExist := true;
...
if (Key = Ord('V'))  and (Shift = [ssCtrl]) or          // This makes sure pasted text assumes the format of whatever it's pasted into.
...
if (TF.Color = clVocab) or (TF.Color = clKeys) and not ((Key = Ord('Z')) and (Shift = [ssCtrl])) then
    begin
    s := '';
    olen := Utf16Length(WMemo.Lines[y]);
    ch := TranslateKeyEvent(Key);
    if not ((Shift = [ssCtrl]) and ((ch = 67) or (ch = 86) or (ch = 88) or (ch = 90))) then
...
if GKeyGroup.Checked[0] or GKeyGroup.Checked[1] then
    ...
    CurP := PChar(GMemo.Lines[y]);        // if S='' then PChar(S) returns a pointer to #0
    ...
    if Found then
        ...
        if s <> '' then
           begin
           GMemo.Lines[y] := Utf8Copy(GMemo.Lines[y], 1, Utf8x) + s + Utf8Copy(GMemo.Lines[y], Utf8x + 1, Utf8Length(GMemo.Lines[y]) - Utf8x);
...
var i: integer;
begin
WordCombo.Items.Clear; WordCombo.Text := Translate('VC');
if TopicCombo.ItemIndex <> - 1 then
  for i := TopicStart(VocabStarts + TopicCombo.ItemIndex) to TopicEnd[VocabStarts + TopicCombo.ItemIndex] do
...
if Enumber >= Sybil_A.Lines.Count then
  if Enumber < Sybil_A.Lines.Count + GMemo.Lines.Count then
     Enumber := Enumber - Sybil_A.Lines.Count
      ...
      begin
      E := E + ' at line ' + IntToStr(Enumber - Sybil_A.Lines.Count - GMemo.Lines.Count + 1) + ' of Sybil B.';
      Enumber := - 1;
      end;
...
if E = '' then                                                 // Out
   ...
   AssignFile(f, HomeDirectory + 'Resources\File transfer\pytopa.rsc');                  // In
   Reset(f);
   ReadLn(f, Qf);
   ReadLn(f, w1);
   ReadLn(f, w2);
...
procedure TForm1.DataEditKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var Utf8x: integer;
begin
...
if MenuEditGrm.Checked then
  ...
  Wipe;
  UpdateScreen;
  Form1.WMemo.Anchors := [];   // This kludge stops WMemo from being visible when it shouldn't be. Idk why.
  Form1.WMemo.Height := 0;
  Form1.WMemo.Width := 0;
  Form1.WMemo.Anchors := [akTop, akLeft, akBottom, akRight];
  end;
...
if AreYouSure() then
  begin
  Wipe; Filename := ''; VcbFileExists := false;
  DataExists := true; {This bit works because wipe doesn't wipe anything, it just forgets that it's there}
  ...
  LowerTopic := VocabStarts; UpperTopic := VocabStarts;
  DoOverrides; UpdateScreen; MakeTopicBox; DisplayVocab(DISPLAY_SELECTED);
  end;
end;
...
DataExists := true; VcbFileExists := false; Filename := ''; UnsavedChangesExist := false;
DoOverrides;
...
if MenuEditGrm.Checked then
  ...
  if GrammarCheck() and OpenGrmDialog.Execute then
    begin
    GrmFileExists := true; SetGrmFilename(Split(ExtractFilename(OpenGrmDialog.Filename), '.grm', 1));
    OpenGrmFile; GrmUnsavedChangesExist := false;
    UpdateScreen;
    end
  end
...
if MenuEditGrm.Checked then
  ...
  if GrammarCheck() then
    begin
    OpenGrmFile; GrmUnsavedChangesExist := false; UpdateScreen;
    end
  end
else if AreYouSure() then
  begin
  Wipe; OpenVcbFile;
  DataExists := true; VcbFileExists := true;
  UpdateScreen;  MakeTopicBox; DisplayVocab(DISPLAY_SELECTED);
  end;
end;
...
begin
if MenuEditGrm.Checked then SaveGrmAs else SaveAs;
end;

procedure TForm1.FormCloseQuery(Sender: TObject; var CanClose: boolean);
begin
CanClose := GrammarCheck and AreYouSure;
if CanClose and not AssignmentExists then SaveRsc(ProgramName, ProgramVersion, ProgramPublisher);
end;
...
LauncherDirectory := Copy(HomeDirectory, 1, Length(HomeDirectory) - 6);  // Trims 'Peach\' off the end.
...
with FormPublish do
  ...
  for i := 1 to ComboBox1.Items.Count do
     ...
     for i := 1 to ComboBox1.Items.Count do
       ...
       if (FontInfo <> '') and
         ((FontInfo = Translate('Default title font')) or
         (FontInfo = Translate('Default body font')) or
         (FontInfo = TitleFont) or
         (FontInfo = MainFont)) then
  ...
  CloseFile(f);
  CopyFile(PChar(HomeDirectory + ProgramName + '.exe'), PChar(HomeDirectory + 'Workspace\' + EditPN.Text + '.exe'), false);       // Copy the program.
  CopyFile(PChar(LauncherDirectory + ProgramName + '.exe'), PChar(HomeDirectory + 'Workspace 2\' + EditPN.Text + '.exe'), false);  // Copy the launcher.
  if FileExists(OpenPictureDialog1.Filename) then
    begin
    CopyFile(PChar(HomeDirectory + 'Resources\Icon\icon.ico'), PChar(HomeDirectory + 'Workspace\icon.ico'), false);
    CopyFile(PChar(OpenPictureDialog1.Filename), PChar(HomeDirectory + 'Resources\Icon\icon.ico'), false);
    UpdateIcon(HomeDirectory + 'Workspace\' + EditPN.Text + '.exe', HomeDirectory + 'Resources\Icon\icon.ico');      // Update the program icon.
    UpdateIcon(HomeDirectory + 'Workspace 2\' + EditPN.Text + '.exe', HomeDirectory + 'Resources\Icon\icon.ico');      // Update the launcher icon.
  ...
  if EditVN.Text = '' then                // For some reason an empty version name causes a fault; this crudely prevents it.
  ...
  WriteLn(f, '[Files]');
  WriteLn(f, 'Source: "' + HomeDirectory + 'Workspace 2\{#MyAppExeName}"; DestDir: "{app}"; Flags: ignoreversion;');
  WriteLn(f, 'Source: "' + HomeDirectory + 'Workspace\{#MyAppExeName}"; DestDir: "{app}\Peach"; Flags: ignoreversion;');
  WriteLn(f, 'Source: "' + HomeDirectory + 'Grammar plugins\*"; DestDir: "{app}\Peach\Grammar plugins"; Flags: ignoreversion; Permissions: users-full;');
  WriteLn(f, 'Source: "' + HomeDirectory + 'Internationalization\*"; DestDir: "{app}\Peach\Internationalization"; Flags: ignoreversion; Permissions: users-full; ');
  WriteLn(f, 'Source: "' + HomeDirectory + 'Licenses\*"; DestDir: "{app}\Peach\Licenses"; Flags: ignoreversion recursesubdirs createallsubdirs; Permissions: users-full;');
  WriteLn(f, 'Source: "' + HomeDirectory + 'Resources\*"; DestDir: "{app}\Peach\Resources"; Flags: ignoreversion recursesubdirs createallsubdirs; Permissions: users-full;');
  WriteLn(f, 'Source: "' + HomeDirectory + 'Vocab lists\*"; DestDir: "{app}\Peach\Vocab lists"; Flags: ignoreversion; Permissions: users-full; ');
  WriteLn(f, 'Source: "' + HomeDirectory + 'libeay32.dll"; DestDir: "{app}\Peach"; Flags: ignoreversion;');
  WriteLn(f, 'Source: "' + HomeDirectory + 'ssleay32.dll"; DestDir: "{app}\Peach"; Flags: ignoreversion;');
  WriteLn(f, 'Source: "' + LauncherDirectory + '\libeay32.dll"; DestDir: "{app}"; Flags: ignoreversion;');
  WriteLn(f, 'Source: "' + LauncherDirectory + '\ssleay32.dll"; DestDir: "{app}"; Flags: ignoreversion;');
  for i := 1 to ListBox1.Items.Count do
      WriteLn(f, 'Source: "C:\Windows\Fonts\' + ListBox1.Items[i - 1] + '"; DestDir: "{autofonts}"; FontInstall: "' + Split(ListBox1.Items[i - 1], '.', 1) + '"; Flags: onlyifdoesntexist uninsneveruninstall');
  WriteLn(f, '[Registry]');
  ...
  WriteLn(f, '[Run]');
  WriteLn(f, 'FileName: "{app}\' + EditPN.Text + '.exe"; Description: "{cm:LaunchProgram,{#StringChange(MyAppName, ''&'', ''&&'')}}"; Flags: nowait postinstall skipifsilent');
  CloseFile(f);
  ...
  if FileExists(OpenPictureDialog1.Filename) then
    ...
    CopyFile(PChar(HomeDirectory + 'Workspace\icon.ico'), PChar(HomeDirectory + 'Resources\Icon\icon.ico'), false);
...
SignedIn := false; Username := ''; Password := '';
SaveRsc(ProgramName, ProgramVersion, ProgramPublisher);
MenuClassroomClick(Form1);
UpdateScreen;
end;
...
with FormRegister do
     ...
     EditEM.Text := '';
     ShowModal;
     if (ModalResult = mrOK) and Valid(EditFN.Text) and Valid(EditLN.Text)
     and Valid(EditUN.Text) and Valid(EditPW.Text) and Valid (EditEM.Text)
     and Add_user(EditUN.Text, EditFN.Text, EditLN.Text, EditPW.Text, EditEM.Text) then
        begin
...
with FormRegister do
     ...
     ShowModal;
     if (ModalResult = mrOK)
     and MailNewPassword(EditUN.Text, EditEM.Text, s) then
         Alert('PSENT');
     end;
end;
...
with FormRegister do
     ...
     if (ModalResult = mrOK)
     and UpdateEmail(Username, EditUN.Caption, EditEM.Caption) then
         begin
         Alert('EUPD');
         end;
     end;
...
with FormRegister do
     ...
     if (ModalResult = mrOK)
     and UpdatePassword(Username, EditUN.Caption, EditEM.Caption) then
         begin
         Alert('PUPD');
...
for i := 0 to Nts.Count - 1 do
   ...
   case c of
       'r':  begin
              result := QuestionDlg(Translate(''), GetItem(Nts.Strings[i], ' : ', 2) + ' (' + GetItem(Nts.Strings[i], ' : ', 1) + ') '
                     + Translate('NApp1') + GetItem(Nts.Strings[i], ' : ', 4) + Translate('NApp2'), 
                     mtCustom, [1, Translate('ButAccept'), 2, Translate('ButDecline')], '');
              if result = 1 then
                 AddStudentToClass(Password, Username, Username, GetItem(Nts.Strings[i], ' : ', 1), GetItem(Nts.Strings[i], ' : ', 3));
              if result = 2 then
                 SendNotification(Password, Username, GetItem(Nts.Strings[i], ' : ', 1), GetItem(Nts.Strings[i], ' : ', 3), '', 'D');
              end;
       'x':  ShowMessage(GetItem(Nts.Strings[i], ' : ', 2) + ' (' + GetItem(Nts.Strings[i], ' : ', 1) + ') ' + Translate('NLeft') + ' ' + GetItem(Nts.Strings[i], ' : ', 4) + '.');
...
if (FormAssignments.ModalResult = mrOK) and AreYouSure then
  ...
  if IsDecimal(GetItem(AssignmentList.Strings[n], ' : ', 4)) then
     ...
     begin
     AssignmentScore := - 1;   // To ensure that a score of 0 will beat it and be uploaded.
...
with FormAO do
  ...
  AvailableLngFiles := FindAllFiles(HomeDirectory + 'Internationalization', '*.lng', true);
  ApplicationLanguageBox.Items.Clear;
  for i := 0 to AvailableLngFiles.Count - 1 do
      ApplicationLanguageBox.Items.Add(Split(ExtractFileName(AvailableLngFiles[i]), '.', 1));
  ...
  if ModalResult = mrOK then
    ...
    if (FontSizeBox3.ItemIndex <> - 1) and (BaseFontSize <> StrToInt(FontSizeBox3.Text)) then
      begin
      BaseFontSize := StrToInt(FontSizeBox3.Text);
      Fontsize3_b := StrToInt(FontSizeBox3.Text)
      end;
    if (ApplicationLanguageBox.ItemIndex <> - 1) and (ApplicationLanguageBox.Text <> AppLang) then
      begin
      ...
      Mainfont_b := MainFont; TitleFont_b := TitleFont; Fontsize3_b := BaseFontSize;
      end;
    SetFonts;
...
with FormLO do
  ...
  if ModalResult = mrOK then
    begin
    for i := 0 to LockdownCount - 1 do if LockdownGroup.Checked[i] then LockStr[i + 1] := 'Y' else LockStr[i + 1] := 'N';
...
with Form1.WMemo do
  ...
  for i := seDatastarts to seLastSetting do
     ...
     nd := Utf16Pos(#13, Text, Translate + 1); if nd = 0 then nd := Utf16Length(Text);
...
//        showmessage('Discard scheme line: '+Scheme.Strings[SchemeLine]);
        ...
        while TF.Size = BaseFontSize do
              ...
              Par := Utf16Copy(Text, Translate + 1, nd - Translate - 1);
              j := j + 1;
              if (TF.Color = clPlain) or (TF.Color = clKBody) or (TF.Color = clQBody) then
                 begin
                 Qtype[j] := '';
                 VcbEntry[1, j] := '¶' + Par;
                 VcbEntry[2, j] := '';
                 ...
                 VcbEntry[2, j] := Utf16Copy(Par, EmdashPosition + 3, Utf16Length(Par) - EmdashPosition - 1);
                 // We protect the token from being turned into a literal [DEFAULT].
                 if (i = q_formats) and (j = TopicStart(q_formats) + 1) then VcbEntry[1, j] := '#[DF]'
                 end;
...
//                      +'Schlurping: '+ par);
           ...
           if IsLeftWideString(Par, Utf8ToUtf16('■')) or IsLeftWideString(Par, Utf8ToUtf16('□')) or IsLeftWideString(Par, Utf8ToUtf16('●')) or IsLeftWideString(Par, Utf8ToUtf16('○'))
               then
                  begin
                  VcbEntry[1, j] := Utf16Copy(Par, 1, 1) + Utf8Copy(Scheme.Strings[SchemeLine], 2, Length(Scheme.Strings[SchemeLine]) - 1)
                  end
           else
             begin
...
with Form1.WMemo do
  ...
  repeat
      ...
      if TF.Size > BaseFontSize then
         ...
         else StringToEntry(Par, j, LastTopic);
         if (StUtf16 <= SelStart) and (SelStart <= StUtf16 + Utf16Length(Par)) then
           begin
           CurTopic := LastTopic;
           CurEntry := j;
...
LowerTopic := VocabStarts; UpperTopic := LastTopic;
MakeTopicBox;
end;
...
LowerTopic := VocabStarts; UpperTopic := LastTopic;
SelectedTopic := - 1;
MakeTopicBox; if DataExists then
              begin
              DisplayVocab(DISPLAY_SELECTED);
              end;
UpdateScreen;
...
LowerTopic := seDatastarts; UpperTopic := seLastSetting;
...
if DataExists then
  begin GKeyGroup.Items[0] := Language(1); GKeyGroup.Items[1] := Language(2) end
else
  begin GKeyGroup.Items[0] := ''; GKeyGroup.Items[1] := '' end;
...
if TopicCombo.ItemIndex = - 1 then
  ...
  for i := TopicStart(VocabStarts + TopicCombo.ItemIndex) to TopicEnd[VocabStarts + TopicCombo.ItemIndex] do
       if not IsLeft(VcbEntry[1, i], '¶') then WordCombo.Items.Add(EntryToString(i));
  if (TopicStart(SelectedTopic) <= SelectedEntry) and (TopicStart(SelectedTopic) <= TopicEnd[SelectedTopic]) then
...
HelpMessage := HelpMessage + 'Peach ' + GetFileVersion + '.' + #13 + #13
 + Translate('CR1') + #13 + #13
 + Translate('CR2') + #13 + #13
 + Translate('CR3') + #13 + #13
 + 'Viva Las Vegas!' + #13 + #13;
...
if ValidateSelection then
  begin
  if AssignmentInProgress and not AssignmentHasResits
      then UploadResult(Password, Username, AssignmentID, 0); // This is so students can't cheat the "no resits" option
  with FormQA do                                               // by restarting the program.
      begin
      Color := Form1.Color;
      Icon := Form1.Icon;
      ...
      if AssignmentInProgress then
          ...
          if AssignmentType = 1 then DisplayVocab(DISPLAY_NONVOCAB);  // It's a half-closed test, we're closing the book.
          end
       else
          begin
      ...
      ShowOptions := false;
      onum := EntryNumber; Qasked := 0; Qright := 0; Qaim := EntryNumber;
      if QOGroup_i = 3 then begin if AssignmentExists then Qaim := AssignmentQuestionLimit else Qaim := 0 end;
...
begin
AssignmentInProgress := false; DoStartTest;
end;
...
begin
AssignmentInProgress := true; DoStartTest;
end;
...
if not AssignmentExists then
   ...
   for i := 0 to TopicBox.Items.Count - 1 do
       ...
       TopicsToDisplay[IndexToTopic(i)] := TopicBox.Checked[i];
          for j := TopicStart(IndexToTopic(i)) to TopicEnd[IndexToTopic(i)] do
                begin
                InUse[j] := TopicsToDisplay[IndexToTopic(i)] and not IsLeft(VcbEntry[1, j], '¶');
                if InUse[j] then  EntryNumber := EntryNumber + 1;
                SomethingChecked := SomethingChecked or InUse[j];
                end;
       end;
   ...
   WMemo.Lines.EndUpdate;
    ...
    for i := 0 to TopicBox.Items.Count - 1 do TopicBox.Checked[i] := TopicsToDisplay[IndexToTopic(i)];
